...
?>
<title><?php echo $data2["nama_user"];?> | Beranda - dunia para event planer</title>
<?php
}
else{
    ?>
<title>Beranda - dunia para event planer</title>
...
<div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        if (@$_SESSION["user"]) {
                        ?>
                        <b><?php echo $data2["nama_user"];?> (<?php echo $data2["user_user"];?>)</b>! <a href="./?p=posting&profil=<?php echo $data2["user_user"];?>"><li class="fa fa-pencil"></li> Buat Postingan</a> Di dunia event planer
                        <?php
                        }
                        else{
                            ?>
                        ...
                        <?php
                            $datapengumuman = mysqli_fetch_array(mysqli_query($db, "SELECT*FROM tegar"));
                        if (@$data2["level_user"] == '0') {
                            echo "<div class='alert alert-warning'>!: <b style='color:darkred;'>$datapengumuman[pengumuman_tegar]</b></div>";
                            if (@$_POST["pengumuman"]) {
                                $pengumuman = $_POST["katapengumuman"];
                                mysqli_query($db, "UPDATE tegar SET pengumuman_tegar='$pengumuman'");
                            }
                            if (@$_POST["bkdaftar"]) {
                                mysqli_query($db, "UPDATE tegar SET izin_reg_tegar='0'");
                            }
                            elseif (@$_POST["tpdaftar"]) {
                                mysqli_query($db, "UPDATE tegar SET izin_reg_tegar='1'");
                            }
...
<?php
                ...
                ?>
                <a href="./?p=user&user=<?php echo $datasqlpost["penulis_post"];?>"><img src="./assets/img/user/user.jpg" style="width:40px;height:40px;border-radius:100%;"> <b><?php echo $datauserpost["nama_user"];?></b></a>
                <?php
                }
                else{
                ?>
...
?>
                ...
                $(document).ready(function(){
                    ...
                    -  <button class="btn btn-info btn<?php echo $datasqlpost["id_post"];?>"><i class="fa fa-comment-o"></i> komen</button><br><br>
                    <blabla<?php echo $datasqlpost["id_post"];?> hidden>
                    <form method="post" action="./inc/komentar.php?p=<?php echo $datasqlpost["id_post"];?>&a=<?php echo $datasqlpost["penulis_post"];?>">
                    
                    <input autofocus required style="border:none;width:100%" name="komentari" placeholder="masukan komentar..." type="text"> 
                    ...
                    ?>
                    <a href="./login?post=<?php echo $url;?>?p=post&id=<?php echo$datasqlpost["id_post"];?>&post_by=<?php echo $datasqlpost["penulis_post"];?>"><i class="fa fa-sign-in"></i> silahkan review jasa kami</a>
                    <?php
                    }
                    ?>
